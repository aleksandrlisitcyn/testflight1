...
def _order_points(pts: np.ndarray) -> np.ndarray:
    """Sort 4 points for perspective transform."""
    s = pts.sum(axis=1)
...
def detect_pattern_roi(img: np.ndarray) -> np.ndarray:
    """
    Extract ROI where the grid is located.
    HEAD-branch version: stable, robust, does not guess too aggressively.
    """
    if cv2 is None:
        return img.copy()
    ...
    # perspective warp when possible
    if len(approx) >= 4:
    ...
    # fallback: bounding box with margin
    x, y, w, h = cv2.boundingRect(contour)
    pad = int(0.02 * max(w, h))

    return img[
        max(y - pad, 0):min(y + h + pad, img.shape[0]),
        max(x - pad, 0):min(x + w + pad, img.shape[1])
    ].copy()


# =====================================================================
#  GRID ESTIMATION LOGIC (HEAD VERSION — STABLE)
# =====================================================================

def _estimate_cells(length: int, projection: np.ndarray) -> int:
    """
    Estimate number of cells along one axis using projection
    of Sobel edges.
    """
    if length <= 0:
        return 1
...
def detect_and_rectify_grid(img: np.ndarray) -> dict:
    """
    HEAD version — accurate, stable and matches your pipeline.
    ...
    # use Sobel for grid frequency detection
    ...
    return {
        "width": int(max(1, cells_x)),
        "height": int(max(1, cells_y)),
    }